...
${aiResult.conclusion || '카드의 메시지를 마음에 새기고 앞으로 나아가세요.'}`;
          ...
          console.error('[Reading] AI interpretation failed:', aiError.code, aiError.message || aiError);

          // 명확한 오류(크레딧, 인증, 쿼터)는 사용자에게 알림
          if (aiError.code === 'AI_CREDIT_DEPLETED' || aiError.code === 'AI_SERVICE_AUTH_ERROR' || aiError.code === 'AI_SERVICE_RATE_LIMIT') {
            return res.status(503).json({
              success: false,
              error: {
                code: aiError.code,
                message: aiError.message,
                fallback: 'TRADITIONAL 모드로 진행하시려면 처음부터 다시 시작해주세요.'
              }
            });
          }

          // 알 수 없는 일시적 오류만 TRADITIONAL로 조용히 폴백
          console.warn('[Reading] Falling back to traditional mode due to:', aiError.code || aiError.message);
